...
if (!config.github_secret || config.github_secret.length !== 40) {
  console.log('Bot.io must use github secret.');
  console.log();
  process.exit(1);
}
...
app.post('/', function(req, res) {
  ...
  //
  case 'release':
    if (!shell.test('-f', './on_release.js')) {
      log('Command not implemented (release)');
      return;
    }

    if (payload.release.draft) {
      log('Ignoring release draft');
      return;
    }
